...
import com.google.common.annotations.NotPublic;
...
public final class MoreCollectors {
   ...
   * Otherwise, throws the exception created by {@code exceptionSupplier}.
   */
  @NotPublic
  @PreferStaticImport // MOE:strip_line
  public static <T extends @Nullable Object> Collector<T, ?, T> onlyElement(
    ...
    return Collector.of(
        () -> new OnlyElementOrThrowState<T>(exceptionSupplier),
        OnlyElementOrThrowState::add,
        OnlyElementOrThrowState::combine,
        OnlyElementOrThrowState::getElement,
        Collector.Characteristics.UNORDERED);
  }
  ...
  @NotPublic
  private static final class OnlyElementOrThrowState<T extends @Nullable Object> {
    boolean hasElement = false;
    T element;
    Supplier<? extends RuntimeException> exceptionSupplier;

    OnlyElementOrThrowState(Supplier<? extends RuntimeException> exceptionSupplier) {
    ...
    OnlyElementOrThrowState<T> combine(OnlyElementOrThrowState<T> other) {
      if (!hasElement) {
        return other;
      } else if (!other.hasElement) {
        return this;
      } else {
        throw exceptionSupplier.get();
      }
